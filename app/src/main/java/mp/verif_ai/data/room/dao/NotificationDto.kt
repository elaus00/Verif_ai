    ...
    fun toDomain(): Notification = Notification(
        ...
        type = when(type.lowercase()) {
            "reply" -> NotificationType.Reply(
                questionId = typeMetadata?.get("questionId") ?: "",
                replyId = typeMetadata?.get("replyId") ?: "",
                replyContent = typeMetadata?.get("replyContent") ?: ""
            )
            "like" -> NotificationType.Like(
                targetId = typeMetadata?.get("targetId") ?: "",
                targetType = typeMetadata?.get("targetType") ?: ""
            )
            "comment" -> NotificationType.Comment(
                ...
                actionType = typeMetadata?.get("actionType") ?: "unknown",
                metadata = typeMetadata ?: emptyMap()
            )
        },
        title = title,
        content = content,
